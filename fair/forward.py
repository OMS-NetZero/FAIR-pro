...
from constants import molwt, lifetime
...
def fair_scm(emissions=False,
             ...
             rt=4.165,
             F2x=3.74,
             C_0=np.array([278., 722., 275.]),
             natural=np.array([0, 202., 10.]),
             iirf_max=97.0,
             restart_in=False,
             restart_out=False,
             tcr_dbl=70.0):
  ...
  # just test with CO2 and methane for now
  ngas = 3
  nF   = 3
  ...
  if restart_in:
    ...
    # numerical method
    R_i[0,:] = a * emissions[0,0] / ppm_gtc   # to add in non-fossil CO2

  # CO2 is a delta from pre-industrial. Other gases are absolute concentration
  C[0,0] = np.sum(R_i[0,:],axis=-1)
  C[0,1:] = C_0[1:] - C_0[1:]*(1.0 - np.exp(-1.0/np.array(lifetime.aslist[1:3]))) + (natural[1:] + 0.5 * (emissions[0,1:])) / emis2conc[1:3]
