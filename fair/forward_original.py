...
def plot_fair(emms,
  ...
  * => Optional argument
  ...
  ax4.set_xlabel('Time ({0})'.format(tuts))
...
#function which calculates the predicted global sea level rise for a given temperature profile 
    #Semi-empirical model based on the paper by Kopp et al. 2016.
def GSL_calc(T,tstep = 1.0,y_0 = 0., c1_0 = 0.0000321521):
    #import numpy as np
    
    integ_len = T.size
    H = np.zeros(integ_len)
    c1 = np.zeros(integ_len)
    temp0 = np.zeros(integ_len)
    
    #setting initial values
    tau1 = 102.     #was 102.
    tau2 = 3392.    #was 3392.
    a1 = 0.007      #was 0.007
    c1[0] = c1_0   #was 0.0000321521
    temp0[0] = 0.061656924  #was 0.061656924
    H[0] = -0.118   #was -0.118
    
    #use the temperature data to get the global sea level rise estimate
    for x in range(1, integ_len):
        c1[x] = c1[x-1]*(1. - tstep/tau2)
        temp0[x] = temp0[x-1] + (T[x-1] - temp0[x-1])*tstep/tau1
        H[x] = H[x-1] + a1*(T[x] - temp0[x]) + c1[x]
    
    return H
    
def GSL_plot(H,tstep=1.0,y_0=0., infig = False, ax1in = None, 
             label = None, colour = 'black', linestyle='-'):
    
    #Third party plotting tools setup ----------
    from matplotlib import pyplot as plt
    plt.style.use('seaborn-darkgrid')
    plt.rcParams['figure.figsize'] = 16, 9
    plt.rcParams['lines.linewidth'] = 1.5
    ...
    time = np.arange(y_0,y_0+H.size,tstep)
    
    ax1.plot(time, H, color=colour, label = label, ls = linestyle)
    ax1.set_ylabel('Global Sea Level (m)')
